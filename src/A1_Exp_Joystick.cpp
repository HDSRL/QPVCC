...
public:
	ExternalComm() : udpComp(LOWLEVEL){
		...
		populate_filter_f(velfilter, af, bf, 3, 2);

		// 2 Hz 
		float aa[3] = {1.00000000,-1.98222893,0.98238545};
		float ba[3] = {0.00003913,0.00007826,0.00003913};
		populate_filter_f(posefilter, aa, ba, 3, 6);
		populate_filter_f(angvelfilter, aa, ba, 3, 1);
		
	}
	virtual ~ExternalComm(){
		clear_filter_d(jointfilter);
		clear_filter_f(velfilter);
		clear_filter_f(posefilter);
		clear_filter_f(angvelfilter);
	}
	...
	FiltStruct_f* angvelfilter    = (FilterStructure_f*)malloc(sizeof(FilterStructure_f));
	FiltStruct_f* posefilter    = (FilterStructure_f*)malloc(sizeof(FilterStructure_f));
	FiltStruct_f* velfilter = (FilterStructure_f*)malloc(sizeof(FilterStructure_f));
...
//	tic(&tset);
	...
	static int fallcnt = 0;
	bool sit = (int)remote.btn.components.B!=0 && (int)remote.btn.components.L2!=0;
	fallcnt += (!sit && fallcnt<5000) ? 1 : 0; 
	if(sit){
		softFall = true;
		setup = true;
		stack.allowStand = true;
		stack.standing = false;
		stack.gait = POSE;
		beginCommand = false;
		fallcnt = 0;
	}
	if( ((int)remote.btn.components.L1!=0) && ((int)remote.btn.components.start!=0) && fallcnt >= 3000 ){
//		std::cout<<"Standing"<<std::endl;
	...
	if (stack.standing){
		if (stack.gait==TROT){
			vel[0] = 1.5f*remote.ly; // x vel
			vel[1] = -0.5f*remote.rx; // y vel
			vel[2] = -2.0f*remote.lx; // yaw vel
			for(int i=0;i<6;++i){
				pose[i] = 0;
			}
			pose[4] = 15.0f*3.14f/180.0f*remote.ry; // pitch angle
		}else{
			float rad2deg = 3.14f/180.0f;
			pose[2] = 0.04f*remote.ly; // delta z
			pose[3] = 30.0f*rad2deg*remote.rx; // roll angle
			pose[4] = 15.0f*rad2deg*remote.ry; // pitch angle
			pose[5] = 15.0f*rad2deg*remote.lx; // yaw angle
			vel[0] = 0; vel[1] = 0; vel[2] = 0;
		}
	}
	// Filter all joystick inputs
	discrete_butter_f(velfilter,vel);
	discrete_butter_f(angvelfilter,&vel[2]);
	discrete_butter_f(posefilter,pose); 

	// Toggle from trot to pose
	static int startCnt = 0;
	static bool startHeld = true;
	static bool prevStartState = 0;
	int startState = (int)remote.btn.components.start;
	startHeld = prevStartState==startState;
	startCnt += (startState==0 && startCnt<1000) ? 1 : 0;
	if ( (startState!=0 && startCnt>=250 && !startHeld) && stack.standing ){
		if ( stack.gait==TROT && (vel[0]+vel[1]+vel[2])<0.02f ){
			stack.gait = POSE;
			std::cout<<"Transitioned to Pose"<<std::endl;
		} else if( stack.gait == POSE && (pose[2]+pose[3]+pose[4]+pose[5])<0.02f ){
			std::cout<<"Transitioned to Trot"<<std::endl;
			stack.gait = TROT;
		}
	}
	startCnt = (startState!=0) ? 0 : startCnt;
	...
	if ( motiontime >= starttime ){
		if (beginCommand & !softFall){
			if (setup){
				...
				loco_obj->updatePoseType(POSE_CMD);
				setup = false;
			}
			...
			if (motiontime<endStand){
				...
				loco_obj->calcTau(q, dq, rotMat, footForce, stack.gait, motiontime);
...
//							cmd.motorCmd[i].tau = 0;
			...
			for (int i = 0; i < 12; ++i){
				...
				cmd.motorCmd[i].dq  = 0.0;
				cmd.motorCmd[i].Kp = 0.0f;
				cmd.motorCmd[i].Kd = ( ( (i+1)%3 ) == 0 ) ? 8.0f : 8.0f;
			}
		}
	}
