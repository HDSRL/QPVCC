...
inline void quat_to_ZYX(double qw, double qx, double qy, double qz, double &roll, double &pitch, double &yaw){
	double aSinInput = -2*(qx*qz - qy*qw);
    roll = atan2( 2*(qy*qz + qx*qw), pow(qw,2)-pow(qx,2)-pow(qy,2)+pow(qz,2) );
    pitch = asin( aSinInput );
    yaw = atan2( 2*(qx*qy + qz*qw), pow(qw,2) + pow(qx,2) - pow(qy,2) - pow(qz,2) );
...
template <typename inputmat, typename outputmat>
inline void quat_to_ZYX(const Eigen::DenseBase<inputmat> &quat, Eigen::DenseBase<outputmat> &eul){
	double roll = 0.0;
	double pitch = 0.0;
	double yaw = 0.0;
	quat_to_ZYX(quat(0),quat(1),quat(2),quat(3),roll,pitch,yaw);
	eul(0) = roll;
	eul(1) = pitch;
	eul(2) = yaw;
}
...
inline void R_ZYX(double roll, double pitch, double yaw, Eigen::Matrix<double,3,3> &R){
	double sroll = sin(roll);
	double croll = cos(roll);
	double spitch = sin(pitch);
	double cpitch = cos(pitch);
	...
	R(1,0) = cpitch*syaw;
	R(2,0) = -spitch;
	R(0,1) = cyaw*spitch*sroll-croll*syaw;
	R(1,1) = croll*cyaw+spitch*sroll*syaw;
	R(2,1) = cpitch*sroll;
	R(0,2) = sroll*syaw+croll*cyaw*spitch;
	R(1,2) = croll*spitch*syaw-cyaw*sroll;
	R(2,2) = cpitch*croll;
}

inline void R_ZYX(Eigen::Matrix<double,3,1> &orient, Eigen::Matrix<double,3,3> &R){
	double roll = orient(0);
	double pitch = orient(1);
	double yaw = orient(2);
	R_ZYX(roll, pitch, yaw, R);
}
